...
func place(gridPos, tile_ref): # Pass 'self' from the tile
	boardBuffer[gridPos] = tile_ref
		
func isValidGridPos(gridPos, requesting_tile):
	# Check if someone ELSE is in this spot
	if boardBuffer.has(gridPos) and boardBuffer[gridPos] != requesting_tile: 
		return false
	if permBoard.has(gridPos):
		return false
	# Bounds check
...
func flushBoardBuffer():
	for gridPos in boardBuffer.keys():
		...
		boardBuffer.erase(gridPos)
		permBoard[gridPos] = tile
...
func get_all_new_words() -> Array:
	...
	var full_board = permBoard.duplicate()
	for pos in boardBuffer:
		full_board[pos] = boardBuffer[pos]
		...
		for axis in ["horizontal", "vertical"]:
			...
			if word.size() >= 2:
				...
				if not signature in detected_word_signatures:
					...
					new_words.append(_convert_to_tile_refs(word, full_board))
	
	return new_words

func _get_word_at_pos(start_pos: Vector2i, axis: String, full_board: Dictionary) -> Array:
	...
	while full_board.has(check_pos):
...
func _convert_to_tile_refs(pos_array: Array, full_board: Dictionary) -> Array:
	var tile_refs = []
	for p in pos_array:
		tile_refs.append(full_board[p])
	return tile_refs
